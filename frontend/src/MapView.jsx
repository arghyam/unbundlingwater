...
// Custom icon for state/district markers with user count
...
// District coordinates for each state (add more districts as needed)
...
const MapView = () => {
  ...
  useEffect(() => {
    const fetchStateData = async () => {
      try {
        const response = await fetch("http://localhost:4000/api/states");
        if (!response.ok) {
  ...
  const fetchDistrictData = async (state) => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/districts/${encodeURIComponent(state)}`
      );
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const displayUserMarkers = () => {
    ...
    userData.forEach((user) => {
      ...
      L.marker([lat, lng], { icon: userIcon })
        .addTo(mapRef.current)
        .bindPopup(popupContent); // Bind the popup here
    });
  };
